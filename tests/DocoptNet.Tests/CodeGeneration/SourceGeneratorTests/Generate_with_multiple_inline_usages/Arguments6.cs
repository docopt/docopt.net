...
partial class Arguments6 : IEnumerable<KeyValuePair<string, object?>>
{
    ...
    public static IParseResult<Arguments6> Parse(IEnumerable<string> args, ParseFlags flags = ParseFlags.None, string? version = null)
    {
        ...
        static IParseResult<Arguments6> Parse(Leaves left)
        {
            var required = new RequiredMatcher(1, left, new Leaves());
            Match(ref required);
            if (!required.Result || required.Left.Count > 0)
            {
                return new ParseElseResult<Arguments6>(new InputErrorResult(string.Empty, Usage));
            }
            ...
            return new ArgumentsResult<Arguments6>(result);
        }

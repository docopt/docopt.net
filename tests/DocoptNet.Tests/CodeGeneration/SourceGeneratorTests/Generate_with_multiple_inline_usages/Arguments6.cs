...
partial class Arguments6 : IEnumerable<KeyValuePair<string, object?>>
{
    public const string Usage = "Usage: my_program (run [--fast] | jump [--high])";

    static readonly IHelpFeaturingParser<Arguments6> Parser = GeneratedSourceModule.CreateParser(Help, Parse);

    public static IHelpFeaturingParser<Arguments6> CreateParser() => Parser;

    static IParser<Arguments6>.IResult Parse(IEnumerable<string> args, ParseFlags flags, string? version)
    {
        ...
        static IParser<Arguments6>.IResult Parse(Leaves left)
        {
            var required = new RequiredMatcher(1, left, new Leaves());
            Match(ref required);
            if (!required.Result || required.Left.Count > 0)
            {
                return GeneratedSourceModule.CreateInputErrorResult<Arguments6>(string.Empty, Usage);
            }
            var collected = required.Collected;
            var result = new Arguments6();

...
partial class Arguments3 : IEnumerable<KeyValuePair<string, object?>>
{
    ...
    public static IParseResult<Arguments3> Parse(IEnumerable<string> args, ParseFlags flags = ParseFlags.None, string? version = null)
    {
        ...
        static IParseResult<Arguments3> Parse(Leaves left)
        {
            var required = new RequiredMatcher(1, left, new Leaves());
            Match(ref required);
            if (!required.Result || required.Left.Count > 0)
            {
                return new ParseElseResult<Arguments3>(new InputErrorResult(string.Empty, Usage));
            }
            ...
            return new ArgumentsResult<Arguments3>(result);
        }

...
partial class Arguments1 : IEnumerable<KeyValuePair<string, object?>>
{
    public const string Usage = "Usage: my_program (run [--fast] | jump [--high])";

    static readonly IParserWithHelpSupport<Arguments1> Parser = GeneratedSourceModule.CreateParser(Help, Parse);

    public static IParserWithHelpSupport<Arguments1> CreateParser() => Parser;

    public static IParser<Arguments1> CreateParser(string version) => Parser.WithVersion(version);

    public static IParser<Arguments1>.IResult Parse(IEnumerable<string> args, ParseFlags flags = ParseFlags.None, string? version = null)
    {
        ...
        static IParser<Arguments1>.IResult Parse(Leaves left)
        {
            var required = new RequiredMatcher(1, left, new Leaves());
            Match(ref required);
            if (!required.Result || required.Left.Count > 0)
            {
                return GeneratedSourceModule.CreateInputErrorResult<Arguments1>(string.Empty, Usage);
            }
            var collected = required.Collected;
            var result = new Arguments1();
